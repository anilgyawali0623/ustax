          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="relative">
              <div className="bg-gradient-to-br from-navy-dark via-navy to-navy-light rounded-3xl aspect-[4/3] flex flex-col items-center justify-center overflow-hidden relative">
                ...
                <div className="font-display text-8xl font-black text-gold relative z-10">2025</div>
...
Our goal is to bridge the gap between complex financial rules and everyday clients by offering clear guidance, accurate services, and personalized support. We are committed to helping clients stay compliant, save money, and achieve financial success.
      ...
      <section className="py-24 bg-slate-50">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <p className="text-gray-500 max-w-2xl mb-14">Credentialed professionals with decades of combined experience across the US, Nepal</p>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {attorneys.map((a) => (
              <div key={a.name} className="bg-white rounded-3xl p-8 shadow-sm border border-slate-100 flex flex-col">
                <div className="flex items-start gap-5 mb-5">
                  ...
                  />
                  <div>
                    <h4 className="font-display text-navy text-lg font-bold mb-1">{a.name}</h4>
                    <p className="text-gold text-xs font-semibold leading-relaxed">{a.title}</p>
                  </div>
                </div>

                <p className="text-gray-600 text-sm leading-relaxed mb-4">{a.bio}</p>
                <p className="text-gray-500 text-sm leading-relaxed mb-6">{a.education}</p>

                <div className="mt-auto pt-5 border-t border-slate-100 space-y-2 text-sm">
                  <div className="flex items-center gap-2 text-gray-600">
                    <span className="text-gold">✉️</span>
                    <a href={`mailto:${a.email}`} className="hover:text-gold transition-colors no-underline text-gray-600">{a.email}</a>
                  </div>
                  <div className="flex items-center gap-2 text-gray-600">
                    <span className="text-gold">📞</span>
                    <a href={`tel:${a.phone}`} className="hover:text-gold transition-colors no-underline text-gray-600">{a.phone}</a>
                  </div>
